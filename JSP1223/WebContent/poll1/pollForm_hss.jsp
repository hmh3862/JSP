...
<body>
	<%
		...
		FileReader fr = new FileReader(path + File.separator + fileName);// -해석 :  fr에 파일경로를 추가한다. path + File.separator + fileName 이게 사실상 (poll\poll.json)이렇게 생각하면된다.   
		...
		//-해석 : gson.fromJson(fr, PollVO.class)는  fr문자열을  pollVO 클래스로 변환시킨다. 즉, poll\poll.json을 읽어서 pollvo클래스에 객체로 집어넣는다. (Gson이 라이브러리기 때문에 가상의 메모리를 사용한다고 보시면됩니다.)	
		fr.close();// -해석 : 썼으니까 닫아야죠?
		
		// out.println(pollVO);
	%>
	<form action="pollOk.jsp" method="post">  <!-- 폼 데이터를 pollOK.jsp로 보낸다. 즉 투표하기를 눌으면 pollOK.jsp로 넘어간다. -->
	<table>
		<tr>
