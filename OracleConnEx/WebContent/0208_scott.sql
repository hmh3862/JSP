...
SELECT * FROM EMP e ;
SELECT DEPTNO FROM EMP e WHERE ENAME = UPPER('Blake') ;
SELECT ENAME , HIREDATE , DEPTNO FROM EMP e WHERE DEPTNO = (SELECT DEPTNO FROM EMP e WHERE ENAME = UPPER('Blake'));
...
SELECT EMPNO , ENAME , SAL FROM EMP e WHERE DEPTNO IN (SELECT DEPTNO FROM EMP WHERE ENAME LIKE '%T%') ORDER BY EMPNO ;
...
SELECT ENAME , JOB , SAL FROM EMP WHERE SAL = (SELECT SAL FROM EMP WHERE ENAME = 'SCOTT') OR SAL = (SELECT SAL FROM EMP WHERE ENAME = 'WARD');
...
SELECT DISTINCT MGR FROM EMP e WHERE MGR IS NOT NULL ; -- 매니저들 사번
